...
|----|-------------------|:----:|:--------:|-----------|:------:|
| F-01 | **Read-aloud (TTS)** — voice reads the question (and options) aloud for pre-readers, auto or 🔊 button. Browser `speechSynthesis`, no backend. *(idea #3)* | M | High | — | Done — auto-reads each question + options in French on render, plus a 🔊 replay button and a mute toggle (remembered per-device via `localStorage`); see `src/tts.js`. |
