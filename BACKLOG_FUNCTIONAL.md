...
| F-31 | **"Mode Merveilleux" (streak unlock)** — a special mode that **unlocks after 10 correct answers in a row**; a reward for sustained focus. **What it is:** a bank of **funny, whimsical questions** (licornes, anniversaires, fêtes…) played with a **special magical visual theme** (e.g. rainbow/sparkle palette, unicorn/birthday decorations, festive wheel). Needs new content (a `Merveilleux` question set, may be served by F-20) and a themed skin in `src/styles/`. **Open questions:** whether the streak counts within one theme run or across the whole session; whether the unlock is one-shot (lost on a wrong answer) or permanent per profile (persist on `profiles`). Pairs with F-05 celebrations for the unlock moment. *(new — user request)* | M | Med | F-10 | Todo |
| F-32 | **Surprise = mix of all themes** — replace the dedicated Surprise question bank (~100 questions in `questions.js`) with a **mix & match drawn from the other 6 themes** (Maths, Lecture, Géographie, Astronomie, Animaux, Arabe). The result card should still show which theme each question came from. Decide: retire or keep the existing Surprise questions (and `tools/fetch-surprise.ps1`); balanced vs purely random draw across themes. Supersedes F-19. *(new — user request)* | S | Med | — | Todo |
