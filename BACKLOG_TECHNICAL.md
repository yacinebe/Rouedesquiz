...
| T-31 | **Interactive backlog control panel** — `/backlog` on Vercel: edit stories in place (text, size, priority, dependencies, status → committed back to `BACKLOG_*.md` on `main`), create the GitHub issue, trigger the Claude Code action, watch a live pipeline checklist (issue → run → PR → preview → merged), and send a refinement after testing the preview (routed to the open PR, else the open issue, else a new follow-up issue). Serverless functions in `api/` (no deps, no build step) + a shared-password gate; needs `GITHUB_TOKEN` (fine-grained PAT) and `BACKLOG_KEY` in Vercel. *(user request — backlog as the workflow cockpit)* | M | High | T-25 | In progress |
