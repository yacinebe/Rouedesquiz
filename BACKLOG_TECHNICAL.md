...
| T-26 | **Device-independent accounts** — account-first gate: welcome (sign in / sign up) → the account's profiles. **Email + password** (Supabase Auth) with a show-password toggle and a forgot-password reset flow; sign out returns to welcome. Cross-device by design (same account anywhere); anonymous/guest play dropped. Config: Confirm-email OFF (or redirect URLs + SMTP for confirm/reset emails). Unblocks a trustworthy F-16 leaderboard, cross-device F-06, and F-17. | L | Med | T-05 | Done |
